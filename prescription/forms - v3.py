...
class PrescriptionDetailForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'drug_name', 'composition', 'dosage',
            'frequency', 'duration', 'food_order',
        ]
        ...
        widgets = {
            ...
            'food_order':  forms.Select(attrs={'class':'form-select', 'data-placeholder':'Food order…'}),
        }

    def __init__(self, *args, user=None, **kwargs):
        super().__init__(*args, **kwargs)
        # Drug name as free text for AJAX autocomplete
        ...
        for field_name in ['dosage', 'frequency', 'duration']:
            ...
            choices = [('', f"Select {field_name.title()}")] + [(p.value, p.value) for p in presets]
